"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var slack_exporter_1 = require("slack-exporter");
var dotenv = require("dotenv");
var yargs_1 = require("yargs");
var helpers_1 = require("yargs/helpers");
var path = require("path");
// Load environment variables from .env file
dotenv.config({ override: true });
// Parse command line arguments similar to main.ts
var args = (0, yargs_1.default)((0, helpers_1.hideBin)(process.argv))
    .options({
    channelId: {
        alias: "c",
        type: "string",
        description: "The ID of the channel to fetch messages from",
        demandOption: true,
    },
    startDate: {
        alias: "s",
        type: "string",
        description: "Start date to fetch messages from (YYYY-MM-DD)",
        demandOption: true,
    },
    endDate: {
        alias: "e",
        type: "string",
        description: "End date to fetch messages to (YYYY-MM-DD)",
    },
    format: {
        alias: "f",
        type: "string",
        description: "Output format (docx or md)",
        choices: ["docx", "md"],
        default: "docx",
    },
    concurrency: {
        alias: "p",
        type: "number",
        description: "Number of concurrent requests to Slack API",
        default: 4,
    },
})
    .parseSync();
function main() {
    return __awaiter(this, void 0, void 0, function () {
        var exporter, startDate, endDate, channelName, isSingleDay, extension, outputPath, result, error_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    exporter = new slack_exporter_1.SlackExporter({
                        token: process.env.SLACK_API_TOKEN || "your-slack-api-token",
                        concurrency: args.concurrency,
                    });
                    _a.label = 1;
                case 1:
                    _a.trys.push([1, 4, , 5]);
                    startDate = args.startDate;
                    endDate = args.endDate || startDate;
                    return [4 /*yield*/, exporter.getChannelName(args.channelId)];
                case 2:
                    channelName = _a.sent();
                    console.log("Working with channel: ".concat(channelName));
                    isSingleDay = startDate === endDate;
                    extension = args.format === "md" ? "md" : "docx";
                    outputPath = path.join("./output", isSingleDay
                        ? "".concat(channelName, "_").concat(startDate, ".").concat(extension)
                        : "".concat(channelName, "_").concat(startDate, "--").concat(endDate, ".").concat(extension));
                    return [4 /*yield*/, exporter.export({
                            channelId: args.channelId,
                            startDate: startDate,
                            endDate: endDate,
                            format: args.format,
                            outputPath: outputPath,
                        })];
                case 3:
                    result = _a.sent();
                    if (result.messageCount === 0) {
                        console.log("No messages found in the specified date range.");
                        return [2 /*return*/];
                    }
                    // Output the path for scripts to use
                    console.log("Output file created: ".concat(result.filePath));
                    return [3 /*break*/, 5];
                case 4:
                    error_1 = _a.sent();
                    console.error("Error in main process:", error_1);
                    process.exit(1);
                    return [3 /*break*/, 5];
                case 5: return [2 /*return*/];
            }
        });
    });
}
// Run the example
if (require.main === module) {
    main().catch(console.error);
}
